...
public:
    ...
    const group info = (
                            ...
                            "info - outputs the session info" % group(
                                value("sess_id", sess_id) % "session unique identifier"
                            )
                    );
    ...
    const parameter wrapped_help = (
                        option("--wrapped-help").set(selected, mode::wrapped_help) % "shows help message for a wrapped application, where <db-file> is determined automatically\n"
                );
    ...
    const group cli = (
                       (commands | version | help | wrapped_help | configure )
                    );
    ...
    void parse(int argc, char* argv[]) const {
        ...
        if (selected == mode::help) {
            std::cout << program_name << " - history manager for Bash" << std::endl << std::endl;

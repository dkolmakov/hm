...
struct HistoryException : UtilException {
  explicit HistoryException(const std::string& _reason) noexcept
      : UtilException("History manager error ", _reason) {}
};

struct LastCommandsSchemaColumnsList {
  const Column<uint64_t> sess_id{"sess_id", "INTEGER PRIMARY KEY"};
...
struct LastCommandsSchema : LastCommandsSchemaColumnsList, Schema {
  const std::string select = "SELECT * FROM " + table_name + " WHERE " +
                             sess_id.binding + " AND " + pwd.binding + ";";
  const std::string insertOrReplace = "INSERT OR REPLACE INTO " + table_name +
                                      " (" + list + ") VALUES (" +
                                      bindingsList + ");";
  const std::string create_table =
      "CREATE TABLE IF NOT EXISTS " + table_name + " (" + listWithTypes + ");";

  LastCommandsSchema() noexcept : Schema("last_commands", sess_id, pwd, cmd) {}
};

struct SessionsSchemaColumnsList {
  const Column<uint64_t> id{"id", "INTEGER PRIMARY KEY"};
...
struct SessionsSchema : SessionsSchemaColumnsList, Schema {
  const std::string select_by_name =
      "SELECT * FROM " + table_name + " WHERE " + name.binding + ";";
  const std::string select_by_id =
      "SELECT * FROM " + table_name + " WHERE " + id.binding + ";";
  const std::string create_table =
      "CREATE TABLE IF NOT EXISTS " + table_name + " (" + listWithTypes + ");";
  const std::string insert = "INSERT INTO " + table_name + " (" +
                             getList(date, name) + ") VALUES (DATETIME(), " +
                             getBindingsList(name) + ");";
  const std::string update = "UPDATE " + table_name + " SET " + name.binding +
                             " WHERE " + id.binding + ";";

  SessionsSchema() noexcept : Schema("sessions", id, date, name) {}
};

struct HistoryFtsSchemaColumnsList {
  const Column<uint64_t> sess_id{"sess_id", ""};
...
struct HistoryFtsSchema : HistoryFtsSchemaColumnsList, Schema {
  const std::string create_table =
      ...
      list + ", tokenize=\"unicode61 tokenchars \'-._/\'\");";
...
struct CommandsSchemaColumnsList {
  const Column<uint64_t> sess_id{"sess_id", "BIGINT"};
  const Column<std::string> date{"date", "TEXT"};
  const Column<std::string> pwd{"pwd", "TEXT"};
  const Column<std::string> cmd{"cmd", "TEXT"};
...
struct CommandsSchema : CommandsSchemaColumnsList, Schema {
  const std::string insert = "INSERT INTO " + table_name + " (" + list +
                             ") VALUES (" + bindingsList + ");";
  const std::string create_table =
      "CREATE TABLE IF NOT EXISTS " + table_name + " (" + listWithTypes + ");";

  CommandsSchema() noexcept : Schema("commands", sess_id, date, pwd, cmd, rc) {}
};
...
class History {
  ...
  std::vector<int> get_sess_id_by_name(const std::string& sess_name) {
    sqlite::Query query(db, sSch.select_by_name);
    std::vector<int> ids;

    query.bind(sSch.name.bName,
               sess_name);  // TODO(dkolmakov): Implement type safe binding
  ...
  std::string logic_phrase_from_ids(const std::vector<int>& ids) {
    std::string phrase = "";

    for (size_t i = 0; i < ids.size(); ++i) {
      phrase += std::to_string(ids[i]);

      if (i + 1 < ids.size()) phrase += " OR ";
    }

    return phrase;
  }

  std::string prepare_select(const std::string& path, const bool recursively,
    ...
    if (sess_phrase.length() && path_phrase.length()) conjunction = "AND";

    return "SELECT * FROM " + ftsSch.table_name + " WHERE " +
           ftsSch.table_name + " MATCH '" + sess_phrase + " " + conjunction +
           " " + path_phrase + "'";
  }

  void insert(const std::string& session, const std::string& datetime,
 ...
 public:
  explicit History(const std::string& path) : db(path) {
    auto sql = cSch.create_table + sSch.create_table + lcSch.create_table +
               ftsSch.create_table +
               "CREATE TRIGGER IF NOT EXISTS history_update AFTER INSERT ON " +
               cSch.table_name + " BEGIN INSERT INTO " + ftsSch.table_name +
               " (" + ftsSch.list + ") VALUES (" +
               ftsSch.getCustomList(
                   [](const ColumnBase& v) { return "new." + v.name; }) +
               "); END;";
    db.exec(sql);
  }
  ...
  void insert_cmd(const std::string& session, const std::string& datetime,
  ...
  void parse_input_file(const std::string& filename,
    ...
    size_t total_lines = std::count(std::istreambuf_iterator<char>(to_count),
                                    std::istreambuf_iterator<char>(), '\n');
    size_t current_line = 0;

    if (total_lines > 0) {
      std::cout << "Loading " << total_lines
                << " commands from the temporary file " << filename
                << std::endl;
